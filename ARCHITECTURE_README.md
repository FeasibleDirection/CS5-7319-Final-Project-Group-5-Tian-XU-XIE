# 多人飞机对战游戏 - 双架构实现

## 🏗️ 架构概览

本项目实现了两种不同的多人游戏架构，用于对比和评估：

### **Architecture A: Server-Authoritative + Event-Driven** ✅ (已实现)
### **Architecture B: P2P Lockstep** ⏳ (待实现)

---

## 📊 Architecture A - 服务器权威 + 事件驱动

### **核心理念**

- **服务器权威**: 所有游戏逻辑在服务器端运行
- **客户端仅发送输入**: 客户端只发送WASD+J按键状态
- **状态广播**: 服务器每40ms（25Hz）广播游戏状态
- **事件驱动**: 模块间通过EventBus解耦通信

### **数据流**

```
┌─────────┐  Input(WASD+J)   ┌─────────────┐
│ Client  │ ──────────────>  │  WebSocket  │
│         │                  │   Handler   │
└─────────┘                  └──────┬──────┘
     ▲                              │
     │                              ▼
     │                    ┌─────────────────┐
     │                    │   Event Bus     │
     │                    │  (InputEvent)   │
     │                    └────────┬────────┘
     │                             │
     │                             ▼
     │                    ┌─────────────────┐
     │                    │  Physics Engine │
     │                    │  - Apply Input  │
     │                    │  - Update Pos   │
     │  State Snapshot    │  - Collision    │
     │  (25Hz)            └────────┬────────┘
     │                             │
     │                             ▼
     │                    ┌─────────────────┐
     │                    │   Game Tick     │
     │ <───────────────── │   Scheduler     │
     └────────────────────│   (25Hz Loop)   │
                         └─────────────────┘
```

### **目录结构**

```
src/main/java/com/projectgroup5/gamedemo/
├── event/                      # 事件驱动核心
│   ├── EventBus.java          # 事件总线
│   ├── GameEvent.java         # 事件接口
│   ├── PlayerJoinedEvent.java
│   ├── InputReceivedEvent.java
│   ├── CollisionDetectedEvent.java
│   ├── ScoreUpdatedEvent.java
│   └── GameEndedEvent.java
│
├── game/                       # 游戏逻辑层
│   ├── GameWorld.java         # 游戏世界状态
│   ├── PlayerEntity.java      # 玩家实体
│   ├── BulletEntity.java      # 子弹实体
│   ├── PlayerInput.java       # 输入数据
│   ├── PhysicsEngine.java     # 物理引擎
│   ├── GameRoomManager.java   # 房间管理
│   └── GameTickScheduler.java # 游戏循环（25Hz）
│
├── websocket/                  # 网络通信层
│   └── GameWebSocketHandler.java
│
└── config/
    ├── WebSocketConfig.java
    └── SchedulingConfig.java
```

### **关键类说明**

#### **1. EventBus (事件总线)**
- 实现发布-订阅模式
- 解耦各模块通信
- 支持类型安全的事件处理

```java
// 订阅事件
eventBus.subscribe(CollisionDetectedEvent.class, event -> {
    // 处理碰撞...
});

// 发布事件
eventBus.publish(new CollisionDetectedEvent(...));
```

#### **2. GameTickScheduler (游戏循环)**
- 固定25Hz（40ms/帧）
- 每帧执行：应用输入 → 更新物理 → 碰撞检测 → 广播状态
- 服务器权威确保公平性

#### **3. PhysicsEngine (物理引擎)**
- 服务器端计算所有移动和碰撞
- 反作弊：验证射速、移动速度
- 圆形碰撞检测（AABB可扩展）

#### **4. GameWebSocketHandler (WebSocket处理)**
- 接收客户端输入
- 广播游戏状态到房间内所有玩家
- 管理连接生命周期

### **客户端实现 (Architecture A)**

文件: `src/main/resources/static/js/game-architecture-a.js`

**核心特点**:
- ✅ **只发送输入**: 不计算位置、碰撞
- ✅ **接收状态**: 从服务器接收完整状态
- ✅ **60FPS渲染**: 插值显示流畅画面
- ✅ **20Hz输入**: 每50ms发送一次输入

```javascript
// 客户端只做这些：
1. 监听键盘输入 (WASD+J)
2. 通过WebSocket发送输入到服务器
3. 接收服务器状态快照
4. 渲染状态（不做物理计算）
```

### **优势**

| 优势 | 评分 | 说明 |
|------|------|------|
| 公平性 | ★★★★★ | 服务器验证所有操作，防作弊 |
| 一致性 | ★★★★★ | 单一真相源，无状态分歧 |
| 可扩展性 | ★★★★☆ | 水平扩展，每个房间独立 |
| 易维护 | ★★★★☆ | 事件驱动，模块解耦 |
| 延迟容忍 | ★★★☆☆ | 需要客户端预测/插值优化 |

### **劣势**

| 劣势 | 影响 | 缓解方案 |
|------|------|----------|
| 服务器负载高 | 中 | 房间分片、限制房间数 |
| 网络延迟影响体验 | 中 | 客户端预测、插值平滑 |
| 需要服务器资源 | 高 | P2P架构可作为补充 |

---

## 📊 Architecture B - P2P Lockstep (未实现)

### **核心理念**

- **去中心化**: 无需专用游戏服务器
- **确定性同步**: 所有客户端运行相同的输入序列
- **锁步协议**: 等待所有玩家输入后统一推进
- **主机迁移**: 临时主机故障时自动切换

### **计划实现**

```
┌─────────┐     Input      ┌─────────┐
│ Client A│ ──────────────>│ Client B│
│         │<───────────────│         │
└────┬────┘     Input      └────┬────┘
     │                          │
     │         ┌─────────┐      │
     └────────>│ Client C│<─────┘
               │  (Host) │
               └─────────┘
               
每帧同步：
1. 收集所有玩家输入
2. 广播给所有peer
3. 所有客户端执行相同输入
4. 验证状态哈希一致性
```

### **对比表**

| 特性 | Architecture A | Architecture B |
|------|----------------|----------------|
| 服务器需求 | 高（游戏逻辑） | 低（仅信令） |
| 延迟 | 50-150ms | 20-50ms（局域网） |
| 公平性 | 极高 | 中（依赖客户端） |
| 防作弊 | 强 | 弱 |
| 扩展性 | 好 | 差（peer数量受限） |
| 网络容错 | 好 | 差（需要全员在线） |

---

## 🚀 如何使用

### **1. 启动服务器**

```bash
mvn spring-boot:run
```

服务器将在 `http://localhost:8080` 启动。

### **2. 访问大厅**

打开浏览器访问: `http://localhost:8080/lobby.html`

### **3. 创建房间并选择架构**

1. 点击"创建房间"
2. 设置玩家数量、地图、胜利条件
3. 等待其他玩家加入
4. 作为房主，你会看到两个开始按钮：
   - **Start (Arch A)**: 服务器权威模式 ✅
   - **Start (Arch B)**: P2P模式 ⏳（未实现）

### **4. 游戏控制**

- **WASD**: 移动
- **J / Space**: 射击
- **目标**: 达到分数或存活最久

---

## 📈 性能指标

### **Architecture A**

| 指标 | 目标 | 实际 |
|------|------|------|
| 游戏Tick频率 | 25Hz | ✅ 25Hz |
| 渲染帧率 | 60FPS | ✅ 60FPS |
| 输入延迟 | <150ms | ✅ 50-100ms |
| 同时在线房间 | 100 | ✅ 支持 |
| 单房间玩家 | 1-4 | ✅ 支持 |

---

## 🔧 技术栈

- **后端**: Spring Boot 3.3.5, Java 17
- **WebSocket**: Spring WebSocket (SockJS fallback可选)
- **调度**: Spring `@Scheduled` (25Hz fixed-rate)
- **前端**: 原生JavaScript + Canvas API
- **数据库**: SQLite (用户、游戏日志)

---

## 📝 API端点

### **大厅相关**

```
POST /api/lobby/rooms/{roomId}/start-architecture-a  # 开始游戏(A架构)
POST /api/lobby/rooms/{roomId}/start-architecture-b  # 开始游戏(B架构)
```

### **WebSocket**

```
WS /ws/game  # 游戏WebSocket连接

消息格式：
{
  "type": "JOIN_GAME" | "PLAYER_INPUT" | "LEAVE_GAME",
  ...
}
```

---

## 📚 参考资料

- [Fast-Paced Multiplayer (Gabriel Gambetta)](https://www.gabrielgambetta.com/client-server-game-architecture.html)
- [Valve's Source Multiplayer Networking](https://developer.valvesoftware.com/wiki/Source_Multiplayer_Networking)
- [Lockstep Protocol](https://en.wikipedia.org/wiki/Lockstep_protocol)

---

## 🎯 未来改进

### **Architecture A 优化**
- [ ] 客户端预测（Client-side Prediction）
- [ ] 服务器和解（Server Reconciliation）
- [ ] 实体插值（Entity Interpolation）
- [ ] 二进制协议（Protobuf/CBOR）
- [ ] Delta压缩（只发送变化）

### **Architecture B 实现**
- [ ] WebRTC DataChannel
- [ ] 确定性物理引擎
- [ ] 状态哈希验证
- [ ] 主机迁移
- [ ] Rollback/Rewind机制

---

## 👥 团队

- **Student 1**: Zhaoyuan Tian - 架构设计
- **Student 2**: Shikuan Xu - 后端开发
- **Student 3**: Jing Xie - 前端开发

---

**版本**: v1.0  
**更新日期**: 2025-11-19  
**架构状态**: Architecture A ✅ | Architecture B ⏳

